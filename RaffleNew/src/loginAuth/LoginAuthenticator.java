	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (user_name.contains("@")) {
				query = session.createQuery("from UserLoginData where user_email = :user_email");
				query.setParameter("user_email", user_name);
				System.out.println("@@@@@@@@@"+user_name);
			} else {
				query = session.createQuery("from UserLoginData where user_name = :user_name");
				query.setParameter("user_name", user_name);
				System.out.println("ELELELELEL"+user_name);
			}
			...
			if (isDataOk & (!list.isEmpty())) {
				if (crypt_password != null) {
					for (int i = 0; i < list.size(); i++) {
						...
						if (((loginData.getUser_name().equals(user_name)) || (loginData.getUser_email().equals(user_name)))
							...
							httpSession.setAttribute("user_id", loginData.getUser_id());
